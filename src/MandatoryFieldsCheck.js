                ...
                <tr>
                  <td>
                    <input
                      ...
                      onChange={() => this.props.toggleMandatorFieldCheck(entry.id)}
                    />
                  </td>
                  <td>
                    {Object.keys(entry).filter(key => key === key.toUpperCase())
                      .map(key => (
                        <div key={`${entry.id}+${key}`}><strong>{key}: </strong>{
                          entry[key] == null
                            ? '<EMPTY>'
                            : key === 'AUTHOR'
                              ? entry[key].map((author, index, authorArray) => (
                                index === authorArray.length - 1
                                  ? <span key={entry.id + author.name}>{author.name}</span>
                                  : <span key={entry.id + author.name}>{author.name} and </span>
                              ))
                              : <span>{entry[key]}</span>
                        }
                        </div>
                    ...
                    {entry.missingRequiredFields.length > 0
                      && <div>Missing required fields: {entry.missingRequiredFields.map(field => (
                        <span key={entry.id + field}>{`${field.toUpperCase()}, `}</span>
                      ))}</div>
                    }
                  </td>
                  <td>
                    {Object.keys(entry.mandatoryFieldsSuggestions).length > 0
                      ? Object.keys(entry.mandatoryFieldsSuggestions).map(key => (
                        key === 'AUTHOR'
                          ? <div key={entry.id + key}><strong>{key}: </strong>
                            {entry.mandatoryFieldsSuggestions.AUTHOR.map((author, index, authorArray) => (
                              <span key={entry.id + author.name}>
                                {`${author.name} ${index === (authorArray.length - 1) ? '' : 'and '}`}
                              </span>
                            ))}</div>
